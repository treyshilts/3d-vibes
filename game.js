  document.addEventListener('DOMContentLoaded', () => {
    ...
    const humanoid = (() => {
      const body = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.3, 1.5, 8), new THREE.MeshStandardMaterial({ color: 0xff0000 }));
      body.position.y = 0.75;

      const head = new THREE.Mesh(new THREE.SphereGeometry(0.3, 8, 8), body.material);
      head.position.y = 1.5;
    ...
    let movingForward = false, movingBackward = false, turningLeft = false, turningRight = false;
    const moveSpeed = 0.05, rotationSpeed = 0.025;
